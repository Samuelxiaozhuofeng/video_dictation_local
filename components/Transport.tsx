...
const Transport: React.FC = () => {
  ...
  const ankiFace = () => {
    switch (ankiStatus) {
      case 'recording': return { tone: 'rose' as const, icon: <Mic size={16} />, label: 'Rec' };
      case 'adding': return { tone: 'ink' as const, icon: <Loader2 size={16} className="animate-spin" />, label: 'Adding' };
      case 'success': return { tone: 'green-soft' as const, icon: <Check size={16} />, label: 'Added' };
      case 'error': return { tone: 'rose-soft' as const, icon: <X size={16} />, label: 'Failed' };
  ...
  return (
    <footer className={`shrink-0 border-t border-line transition-colors ${recording ? 'bg-rose-soft' : 'bg-page'}`}>
      <div className="px-4 sm:px-6 pt-3">
        ...
        <div className="mt-3 pb-3 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <Btn square flat onClick={() => actions.onSkip('prev')} title="Previous line (Ctrl+←)"><SkipBack size={18} /></Btn>
            <Btn square flat onClick={() => actions.onReplayCurrent()} title="Replay line (Shift+Space)"><RotateCcw size={18} /></Btn>
            <Btn square tone="green" onClick={actions.onTogglePlay} title={isPlaying ? 'Pause (Space)' : 'Play (Space)'} className="!rounded-full">
              {isPlaying ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" className="ml-0.5" />}
            </Btn>
            <Btn square flat onClick={() => actions.onSkip('next')} title="Next line (Ctrl+→)"><SkipForward size={18} /></Btn>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Btn square flat onClick={actions.onToggleSaveCurrent} title={isCurrentSaved ? 'Unsave line' : 'Save line'} className={isCurrentSaved ? '!bg-ochre-soft !text-ochre' : ''}>
              <Bookmark size={18} fill={isCurrentSaved ? 'currentColor' : 'none'} />
            </Btn>
            {ankiReady && (
              <Btn size="md" flat={ankiStatus === 'idle'} tone={af.tone} disabled={ankiStatus !== 'idle'} onClick={actions.onAddToAnki} title="Send this line to Anki (Alt+N)">
                {af.icon} <span className="hidden sm:inline">{af.label}</span>
              </Btn>
            )}
            <Btn square flat onClick={() => actions.onSetVolume(volume === 0 ? 1 : 0)} title={volume === 0 ? 'Unmute' : 'Mute'}>
      ...
      <div className={`border-t border-line px-4 sm:px-6 py-1.5 text-[11px] ${recording ? 'bg-rose text-page' : 'bg-paper text-mute'}`}>
        {recording ? (
          <span className="blink">● Recording audio clip for Anki</span>
        ) : (
          <span className="hidden md:inline">Space play/pause · Shift+Space replay · Ctrl+←/→ lines · Enter next{learningMode === LearningMode.DICTATION ? ' · Ctrl+X peek' : ''} · Alt+N Anki</span>
        )}
      </div>
    </footer>
  );
