...
- [ ] 令牌：浅色 #FAFAFA / 白 / #1C1917 / 朱红 #E8492B；字体 Instrument Sans + Source Serif 4（打包，卸 newsreader）
- [ ] 硬编码颜色清扫（rgba 黄、深色阴影）
- [ ] 通用组件 ui.tsx（Btn / Seg / Menu / Dialog 圆角、胶囊）
- [ ] 顶栏：居中胶囊导航 + 右上朱红 +，左侧给红绿灯留位
- [ ] 首页：大封面 + 白条（播放/标题/按句进度/…），其他视频封面一排
- [ ] 练习页：视频上半屏 + 白面板；时长时间轴；控制条；更多面板；查词浮卡；答案错词
- [ ] 句子库 / 单词库（合并复习页 + 卡片页）
- [ ] 复习中页面沿用影院布局
- [ ] 其余：设置、弹窗、段完成、空状态、模糊模式
- [ ] tsc + test-*.mjs
- [ ] 浏览器实测 + 截图评审（最多 3 轮）
- [ ] 快捷键逐个过
- [ ] docs / CLAUDE.md 风格描述更新
- [ ] codex-review + npm run release + 验收路径
