...
fs.rmSync('slides-shots/flow', { recursive: true, force: true });
fs.mkdirSync('slides-shots/flow');
...
function aiReply(prompt) {
  if (prompt.includes('最值得学')) { // 拆开教我：单句，或导入后整批预拆（{lines:[…]}）
    if (prompt.includes('"lines"')) return { lines: prompt.split(/第 \d+ 句（/).slice(1).map(b => aiReply('最值得学\n' + b)) };
    ...
    return { lang: 'en', points: points.length ? points : [{ from: 0, to: 0, note: w[0] + '：示例' }] };
  }
...
const shot = name => p.screenshot({ path: `slides-shots/flow/${name}.png` });
const manifest = { viewport: [W, H], typing: [] };

await p.goto('http://localhost:3000');
await p.evaluate(() => { localStorage.setItem('linguaclip_ai_config', JSON.stringify({ apiKey: 'demo', model: 'deepseek-chat', baseUrl: 'https://api.deepseek.com/v1' }));
...
manifest.startBtn = await p.getByRole('button', { name: /生成字幕/ }).boundingBox();
await p.getByRole('button', { name: /生成字幕/ }).click();
...
await p.getByText(/拆开练/).first().waitFor({ timeout: 15000 }).catch(async e => { await shot('DEBUG'); console.log(await p.evaluate(() => document.body.innerText.slice(0, 1500))); throw e; });
...
await p.getByRole('button', { name: '复习' }).first().click(); await p.waitForTimeout(700);
await shot('saved');
...
fs.writeFileSync('slides-shots/flow/manifest.json', JSON.stringify(manifest, null, 1));
